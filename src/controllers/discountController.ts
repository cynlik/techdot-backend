...
export class DiscountController {
		...
		for (let i = 0; i < lengthProducts; i++) {
			...
			if (product.onDiscount && isActive) {
				throw new CustomError(
					HttpStatus.CONFLICT,
					`O produto ${product.name} já tem um desconto aplicado.`
				);
			}

			let newPrice = isActive
				? product.price - product.price * discountDecimal
	...
	public addProductToDiscount = async (
		req: Request,
		res: Response,
		next: Function
	) => {
		...
		try {
			...
			if (!discount) {
				return next(
					new CustomError(HttpStatus.NOT_FOUND, "Discount not found")
				);
			}
			...
			if (discount.isActive) {
				return next(
					new CustomError(
						...
						"Discount needs to be disabled to update"
					)
				);
			}
	...
	public removeProductFromDiscount = async (
		req: Request,
		res: Response,
		next: Function
	) => {
			...
			if (index !== -1) {
				...
				return next(
					new CustomError(
						...
						"Product not found in applicableProducts"
					)
				);
			}
		} catch (error) {
			...
			if (!discount.isPromoCode) {
				if (discount.applicableProducts.length < 1) {
					return next(
						new CustomError(
							...
							"Para ativar o desconto é necessário adicionar produtos"
						)
					);
				}
				...
				if (discount.isActive) {
					if (isActive) {
						...
						try {
							const updatedPrices = await this.updateProducts(
								discount,
								discountDecimal,
								lenghtProducts,
								false
							...
							return res
								...
								.send({ updatedPrices, updateDiscount });
						} catch (error) {
							return next(error);
						}
					}
					...
					if (isActive) {
						...
						return res
							.status(HttpStatus.OK)
							.send({ message: "O Desconto já está desativado!" });
					}
				}
			}
		} catch (error) {
	...
	public updateDiscount = async (
		...
		try {
			...
			return res
				.status(HttpStatus.OK)
				.json({ message: "Desconto Atualizado: ", updateDiscount });
		} catch (error) {
			next(Error(error));
		}
	};
	...
	public deleteDiscount = async (
		...
		try {
			...
			return res
				.status(204)
				.json({ message: "Discount successfully deleted!" });
		} catch (error) {
			next(Error(error));
		}
	};

	public getDiscountByID = async (
		req: Request,
		res: Response,
		next: Function
	) => {
	...
	public getDiscountByName = async (
		req: CustomRequest,
		res: Response,
		next: Function
	) => {
		try {
			...
			if (discounts.length === 0) {
				next(
					new CustomError(HttpStatus.NOT_FOUND, "Nenhum desconto encontrado.")
				);
			} else {
