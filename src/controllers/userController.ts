...
export default class UserController {
	...
	public getWishList = async (
		...
		try {
			...
			if (!user.wishList || user.wishList.length === 0) {
				res
					.status(HttpStatus.OK)
					.json({ message: SUCCESS_MESSAGES.EMPTY_WISH_LIST });
			} else {
				res.status(HttpStatus.OK).json({
		...
		try {
			...
			if (newUser.country && !this.isValidCountry(newUser.country)) {
				return next(
					new CustomError(
						HttpStatus.BAD_REQUEST,
						ERROR_MESSAGES.INVALID_COUNTRY
					)
				);
			}
			...
			if (newUser.cart && !this.isValidCart(newUser.cart)) {
				return next(
					new CustomError(
						HttpStatus.BAD_REQUEST,
						ERROR_MESSAGES.INVALID_CART_FORMAT
					)
				);
			}
	...
	public changeView = async (
		...
		try {
			...
			if (!user) {
				return next(
					new CustomError(HttpStatus.UNAUTHORIZED, ERROR_MESSAGES.NO_USER)
				);
			}
