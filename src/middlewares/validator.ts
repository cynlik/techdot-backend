...
class Validator {
	...
	static validateIds(
		...
		return async (req: Request, res: Response, next: NextFunction) => {
			for (const { paramName, model, type, isOptional } of idsInfo) {
				...
				if (!isOptional && !id) {
					return next(
						new CustomError(HttpStatus.BAD_REQUEST, `${type} ID is required`)
					);
				}
	...
	static validateTokenMatch(
		queryTokenParam: string,
		userTokenField: keyof IUser
	) {
		return async (req: Request, res: Response, next: NextFunction) => {
			...
			if (!token) {
				return next(
					new CustomError(HttpStatus.BAD_REQUEST, "Token is required")
				);
			}
			...
			try {
				...
				if (!user) {
					return next(
						new CustomError(HttpStatus.BAD_REQUEST, "Invalid or expired token")
					);
				}
